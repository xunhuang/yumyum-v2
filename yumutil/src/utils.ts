...
export async function addressMatch(
  street_a: string,
  street_b: string,
  city: string,
  state: string
  ...
  if (!usps_street_a || !usps_street_b) {
    ...
    const standardized_address_b =
      await get_standardized_US_address_from_google(street_b, city, state);
...
export async function get_longlat_from_address(
  address: string,
  city: string,
  state: string
) {
...
export async function setVenueGPS(
  venue_key: string,
  longitude: number,
  latitude: number
): Promise<any> {
...
export async function get_standardized_US_address_from_google(
  ...
  console.log(
    "Google search: No formatted address found for ",
    address,
    city,
    state
  );
...
export async function GoogleIsThisPlaceClosed(
  name: string,
  address: string,
  city: string,
  state: string
  ...
  if (result.candidates.length !== 1) {
    console.log(
      "GoogleFindPlaceFromText found multiple candidates, returning undecided ",
      result
    );
  ...
  const candidateName = result.candidates[0].name;
  if (venueNameSimilar(name, candidateName)) {
    return false;
  }

  console.log("Google Search - name not matched, returning undecided");
  console.log(result);
