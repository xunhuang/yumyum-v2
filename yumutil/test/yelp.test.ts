...
describe("Yelp", () => {
  ...
  it("getYelpBusinessDetails", async () => {
    const result = await getYelpBusinessDetails("trabocco-kitchen-and-cocktails-alameda");
    expect(result).not.toBeNull();
  ...
  it("yelp_find_reservation", async () => {
    ...
    const result = await yelp_find_reservation(data2.slug, data2.businessid, data2.longitude, data2.latitude, date, 2, "dinner");
    expect(result).not.toBeNull();
  });

  it("yelp_basic_search_and_validate", async () => {
    const result = await yelp_basic_search_and_validate(data2.name, data2.longitude, data2.latitude, data2.address, data2.city, data2.region);
    expect(result).not.toBeNull();
  ...
  it("yelp_basic_search_and_validate_should_not_find_as_not_on_yelp", async () => {
    const result = await yelp_basic_search_and_validate(data.name, data.longitude, data.latitude, data.address, data.city, data.region);
