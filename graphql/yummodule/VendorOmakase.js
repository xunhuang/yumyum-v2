...
class VendorOmakase extends VendorBase {
    vendorID() {
        return "omakase";
    }
    ...
    async venueSearch(venue, date, party_size, timeOption) {
        ...
        Object.values(types).map(mytype => {
            mytype.map(s => slots.push(s));
        });
        ...
        slots = slots.filter(s => s.guests_count_option_values.includes(party_size));

        let actualslots = [];

        slots = slots.map(s => {
            s.time_options.map(timeslot => {
...
exports.VendorOmakase = VendorOmakase;