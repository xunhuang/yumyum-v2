...
describe('Resy System Test', () => {
    ...
    describe('Search entity by name and long/lat', () => {
        it('should find an entity with exact match', async () => {
            const search_result = await resy.entitySearchExactTerm("Aurum", -122.1156105, 37.3801255, null);
        ...
        it('should not find an entity with complete garbage', async () => {
            const search_result = await resy.entitySearchExactTerm("adadadfsfsdf2weweweAurum", -122.1156105, 37.3801255, null);
        ...
        //     const search_result = await resy.entitySearchExactTerm("Au", -122.1156105, 37.3801255, null);
        //     expect(search_result.urlSlug).toEqual("aurum");
        // })
        ...
        it('list of entity (like fail because 5 second limit)', async () => {
            for (const entity of nyc) {
                const search_result = await resy.entitySearchExactTerm(entity.name, entity.longitude, entity.latitude, null);
                if (search_result) {
