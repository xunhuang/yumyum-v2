...
functions.http("resy_reservation_find", async (req, res) => {
  const { url_slug, date, party_size, timezone, businessid } = req.query;

  let result = await resy_redis(url_slug, date, party_size, timezone);
  if (result === null) {
    console.log(" no redis data - looking it up realtime  ");
    result = await findResyReservation(
      date,
      party_size,
      businessid,
      timezone,
    ...
    console.log("redis success for ", req.query);
  }
...
async function resy_redis(url_slug, date, party_size, time_zone) {
  const key = resy_day_key(url_slug, date, party_size);
